...
class AuboTrajectory():
    def __init__(self,Aubo_IP,AuboWorkSpaceLen,AuboWorkSpaceWidth,Datafilename):
        ...
        self.MatlabData=None
        self.Get_data_from_Matlab()
        self.AuboSDKInstance=None
    def Init_node(self):
    ...
    def Get_data_from_Matlab(self):
        current_path = os.path.abspath(".")
        # print(current_path)
        data_path = os.path.join(current_path, "src/spraying_robot/data/"+self.Datafilename)
        load_data = sio.loadmat(data_path)

        self.MatlabData=load_data['room_plane_base_endpoint_cell'][0]#[WallNum][0]#[0]#第一个零取出来的是四面墙的数组，第二个零取出的是第一面墙的数据，
    
    def Get_data_From_Memory(self,WallNum,RectangleNum):
        #self.Get_data_from_Matlab()
        return self.MatlabData[WallNum][0][RectangleNum]                                                           #      第三个零取出的是第一面墙需要的数据，第四个零是第一面墙第一个方块路径
    def Cacula_Aubo_MovePlan_From_Rectangle_Planning(self,WallNum,RectangleNum,StartPointRad):
        RectangleData=self.Get_data_From_Memory(WallNum,RectangleNum)
        Planning_path={}
        startpoint=StartPointRad
        for i in range(len(RectangleData)):
            PosTemp=[]
            # print tuple(RectangleData[i])
            rpy2qutaneion=tuple(self.AuboSDKInstance.rpy_to_quaternion(tuple(RectangleData[i])[3:]))
            Position=tuple(RectangleData[i])[:3]
            for ii in Position:
                PosTemp.append(ii/1000)#m
            
            ik_result = self.AuboSDKInstance.inverse_kin(StartPointRad, tuple(PosTemp), rpy2qutaneion)
...
def main():
    ...
    Aub=AuboTrajectory(IP,1.1,0.7,DataFilename)
    Aub.Init_node()
    rate = rospy.Rate(ratet)
    flag_roation=0
    count=0
    #print(Aub.Get_data_From_Memory(0,0))
